...
describe('test access level 3 endpoints', () => {
    ...
    test('test endpoint get lights', async () => {
        const res = await axios.get('http://localhost:5000/api/l3/lights', { headers: { authorization: token } })
        expect(res.data['status']).toBe(true)
    })

    test('test endpoint route request', async () => {
        const res = await axios.get('http://localhost:5000/api/l3/lights/routedata/requests', { headers: { authorization: token } })
        expect(res.data['status']).toBe(true)
    })
